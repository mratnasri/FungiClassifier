...
gram_img_dir = '../../Gram Stain Images_ver2'
M=150
N=150
...
def img_crops(path,folder,label):
    ...
    for fn in img_names:
        ...
        crops = [img[x:x+M,y:y+N] for x in range(0,img.shape[0]-M+1,M) for y in range(0,img.shape[1]-N+1,N)]
        ...
        for crop in crops:
            ...
            crop_name=label+'-'+str('{:02}'.format(img_num))+'_'+str('{:03}'.format(num))+'.jpg'
            ...
            if (cv2.countNonZero(mask) >=80/100*N*M):
                ...
                i=i+1
                #temp=crop
                cv2.imwrite(folder+'/'+label+'/'+label+'-'+str('{:02}'.format(img_num))+'_'+str('{:03}'.format(i))+'.jpg', crop)
                ...
                #cv2.waitKey(0)
                cv2.imwrite(folder+'_contrast_stretched/'+label+'/'+label+'-'+str('{:02}'.format(img_num))+'_'+str('{:03}'.format(i))+'_contrast_stretched.jpg', crop)
...
for label in os.listdir(gram_img_dir):
    ...
    folder = '../../Gram Stain Images_ver2_Crops_ver2'
