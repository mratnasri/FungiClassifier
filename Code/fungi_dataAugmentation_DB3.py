...
def noise_img(img):
    ...
    noise = tf.random_normal(
        shape=original_size, mean=0.0, stddev=1.0, dtype=tf.float32)
...
def DA_handler(path, folder, label):
    ...
    for fn in img_names:
        ...
        cropped = 255*(cropped-tf.math.reduce_min(cropped)) / \
            (tf.math.reduce_max(cropped)-tf.math.reduce_min(cropped))
        cropped = tf.cast(cropped, dtype=tf.uint8)
        cropped_img = tf.image.encode_jpeg(cropped)
        crop_write = tf.write_file(
            folder+'/'+label+'/'+name.replace('.jpg', '_cropped.jpg'), cropped_img)
        rotated = rotate_img(img)
        rotated = 255*(rotated-tf.math.reduce_min(rotated)) / \
            (tf.math.reduce_max(rotated)-tf.math.reduce_min(rotated))
        rotated = tf.cast(rotated, dtype=tf.uint8)
        rotated_img = tf.image.encode_jpeg(rotated)
        rotate_write = tf.write_file(
            folder+'/'+label+'/'+name.replace('.jpg', '_rotated.jpg'), rotated_img)
        noised = noise_img(img)
        noised = 255*(noised-tf.math.reduce_min(noised)) / \
            (tf.math.reduce_max(noised)-tf.math.reduce_min(noised))
        noised = tf.cast(noised, dtype=tf.uint8)
        noised_img = tf.image.encode_jpeg(noised)
        noise_write = tf.write_file(
            folder+'/'+label+'/'+name.replace('.jpg', '_noise.jpg'), noised_img)
        sess.run(crop_write)
...
for label in os.listdir(gram_img_dir):
    ...
    folder = '../../Gram Stain Images_ver3_Crops_DA'
